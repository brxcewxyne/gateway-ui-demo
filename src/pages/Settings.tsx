  ...
  return (
    <div>
      ...
      <div className="card p-4 min-h-[300px] text-[13px]">
        ...
        {tab === 'Teams' && <div className="text-[#8d99ae]">Manage members + allowed models per team. Budgets live in <b className="text-white">Resources →</b> (single source of truth).</div>}
        {tab === 'Budgets' && <div className="text-[#8d99ae]">Budgets are edited in Resources so spend + action stay together. This tab mirrors read-only totals.</div>}
        {tab === 'Routing' && <div className="text-[#8d99ae]">Default mode, JEV threshold slider, fallback chain. Live editor lives in <b className="text-white">Routing →</b>.</div>}
        {tab === 'Policies' && <div className="text-[#8d99ae]">Policy CRUD mirrors Security. Create there to keep approvals in context.</div>}
        ...
        {tab === 'Notifications' && <div className="max-w-[420px] flex flex-col gap-2"><label className="text-[12px] text-[#8d99ae]">Slack webhook<input className="input mt-1" placeholder="https://hooks.slack.com/…" /></label><label className="text-[12px] text-[#8d99ae]">Alert threshold %<input className="input mt-1" defaultValue={80} type="number" /></label></div>}
      </div>
    </div>
  );
}
