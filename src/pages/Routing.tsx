    ...
    <div>
      <PageHead eyebrow="Model Routing" title="Routing — why this model?" sub="JEV classifies task + difficulty (mocked, swappable API). Gateway applies policy + budget, then picks the model. Every step is explainable."
        right={<>
          <div className="flex items-center gap-1 card px-2 py-1">
            <button className={`text-[12px] px-2 py-1 rounded ${mode === 'auto' ? 'bg-blue-600 text-[#0E1626]' : 'text-[#5B6B82]'}`} onClick={() => setMode('auto')}>● Auto</button>
            <button className={`text-[12px] px-2 py-1 rounded ${mode === 'manual' ? 'bg-blue-600 text-[#0E1626]' : 'text-[#5B6B82]'}`} onClick={() => setMode('manual')}>○ Manual</button>
          </div>
      ...
      <div className="grid lg:grid-cols-[1.2fr_1fr] gap-3">
        <div className="card p-4">
          ...
          {out ? (
            <div className="mt-4 border-t border-[#E3E9F2] pt-3">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                <div className="card p-2.5"><div className="kpi-label">Task</div><div className="text-[#0E1626] text-[14px]">{out.task}</div></div>
                <div className="card p-2.5"><div className="kpi-label">Difficulty</div><div className="text-[#0E1626] text-[14px]">{out.difficulty} <span className="text-[#5B6B82] text-[12px]">conf {(out.confidence * 100).toFixed(0)}%</span></div></div>
                <div className="card p-2.5"><div className="kpi-label">Policy</div><div className="text-[13px] text-[#0E1626]">{out.policy}</div></div>
                <div className="card p-2.5"><div className="kpi-label">Budget</div><div className="text-[13px] text-[#0E1626]">{out.budgetNote}</div></div>
              </div>
              <div className="mt-3">
                ...
                {out.candidates.map(c => (
                  <div key={c.modelId} className={`card p-2.5 mb-1.5 flex items-center justify-between ${c.modelId === out.recommendation ? 'border-blue-500/60' : ''}`}>
                    <div><div className="text-[#0E1626] text-[13.5px]">{c.modelName} {c.modelId === out.recommendation && <span className="badge !text-[#2F7DE1] !border-blue-500/50 ml-1">✓ selected</span>}</div>
                      <div className="text-[12px] text-[#5B6B82]">{c.note}</div></div>
                    <div className="text-right text-[12px] mono text-[#5B6B82]">{fmt$(c.estCost)} · {c.estLatency}s</div>
                  </div>
                ))}
              </div>
              <div className="card p-3 mt-2 !border-blue-500/40 bg-blue-500/[0.06]">
                <div className="text-[13px] text-[#0E1626] font-medium">Reason: {out.reason}</div>
                <div className="flex gap-2 mt-2">
            ...
            <div className="mt-4 text-[13px] text-[#5B6B82]">Run routing to see Task → Difficulty → Policy → Budget → Candidates → Selected + reason. This panel is the CORE explainability loop.</div>
          )}
        </div>

        <div className="flex flex-col gap-3">
          <div className="card p-4">
            <div className="text-[13px] font-medium text-[#0E1626] mb-2">Model priority & fallback ({mode})</div>
            {[...models].sort((a, b) => a.fallbackPriority - b.fallbackPriority).map(m => (
              <div key={m.id} className="flex items-center justify-between py-1.5 border-b border-[#E8EDF4] last:border-0">
                <span className="text-[13px]"><span className="mono text-[#5B6B82]">#{m.fallbackPriority}</span> <Link className="text-[#0E1626] hover:text-[#2F7DE1]" to="/models">{m.name}</Link></span>
            ...
            <div className="text-[12px] text-[#5B6B82] mt-2">Fallback chain auto-applies on error/latency. Manual mode pins the top-priority allowed model.</div>
          </div>
          <div className="card p-4">
            <div className="text-[13px] font-medium text-[#0E1626] mb-2">Routing rules</div>
            {routingRules.map(r => (
              <div key={r.id} className="flex items-center justify-between gap-2 py-1.5 border-b border-[#E8EDF4] last:border-0">
                <div><div className="mono text-[12px] text-[#0E1626]">{r.cond}</div><div className="text-[11px] text-[#5B6B82]">{r.action}</div></div>
                <button className={`text-[11px] px-2 py-1 rounded border ${r.on ? 'border-emerald-500/50 text-[#047857]' : 'border-[#D3DDE9] text-[#5B6B82]'}`} onClick={() => toggleRule(r.id)}>{r.on ? 'ON' : 'OFF'}</button>
              </div>
            ))}
            <div className="flex gap-2 mt-2">
