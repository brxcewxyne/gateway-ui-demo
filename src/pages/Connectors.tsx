...
export default function Connectors() {
  ...
  return (
    <div>
      <PageHead title="MCP / Connectors" sub="What can agents touch? The gateway decides: model requests, gateway allows. Click a node for tools + permissions."
      ...
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-3">
        ...
        <div className="card p-4">
          ...
          <div className="text-[12px] text-[#8b94a3] mono mt-1">{cur.kind} · {cur.server} · {cur.calls.toLocaleString()} calls · err {cur.errRate}% · {cur.lastSeen}</div>
          ...
          {cur.tools.map(t => (
            <div key={t.name} className="card p-2.5 mb-1.5 flex items-center justify-between">
              <div><div className="mono text-[12.5px] text-white">{t.name} <span className="text-[#8b94a3]">{t.readWrite}</span></div>
                <div className="text-[11.5px] text-[#8b94a3]">risk {t.risk} · {t.access}</div></div>
          ...
          <div className="text-[12px] text-[#8b94a3] mt-2">Permission change → <Link className="text-blue-400" to="/security">Security</Link> + <Link className="text-blue-400" to="/audit">Audit log</Link>.</div>
        </div>
      </div>
    </div>
  );
