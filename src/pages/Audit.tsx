    ...
    <div>
      ...
      <div className="card p-3 mb-3 flex flex-wrap gap-2">
        ...
        {['all', 'allow', 'blocked', 'fallback'].map(d => (
          <button key={d} className={`btn !py-1 ${f.decision === d ? '!border-blue-500 !text-white' : ''}`} onClick={() => setF({ ...f, decision: d })}>{d}</button>
        ))}
      ...
      <div className="card p-3 mt-3">
        <div className="flex items-center justify-between mb-2">
          <div className="text-[13px] text-white font-medium">Key & governance events ({keyRows.length})</div>
          <Link to="/keys" className="text-[12px] text-blue-400">Open Keys →</Link>
        </div>
        {keyRows.map(e => (
