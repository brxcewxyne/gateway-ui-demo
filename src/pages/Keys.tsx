...
function Modal({ children, onClose, wide }: { children: React.ReactNode; onClose: () => void; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className={`relative card !bg-white/80 p-5 w-full ${wide ? 'max-w-[560px]' : 'max-w-[440px]'} max-h-[88vh] overflow-y-auto`} onClick={e => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block text-[12px] text-[#5B6B82]">{label}<div className="mt-1">{children}</div></label>;
}
...
function CreateWizard({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} wide>
      <div className="text-[#0E1626] font-medium text-[15px]">Create key {step <= 4 && <span className="text-[#5B6B82] font-normal">· step {step}/4</span>}</div>
      {step <= 4 && <div className="flex gap-1 mt-2">{[1, 2, 3, 4].map(i => <div key={i} className={`h-1 flex-1 rounded ${i <= step ? 'bg-blue-500' : 'bg-[#E2E9F3]'}`} />)}</div>}
      ...
      {step === 3 && <div className="card p-3 mt-4 text-[13px]">
        {[['Name', form.name || '(auto)'], ['Provider', form.provider], ['Team', form.team], ['Environment', form.env], ['Models', form.models], ['Scope', form.scope], ['Quota', form.quota], ['Budget', form.budget], ['Expires', form.expires]].map(([k, v]) => (
          <div key={k} className="flex justify-between py-1 border-b border-[#E8EDF4] last:border-0"><span className="text-[#5B6B82]">{k}</span><span className="text-[#0E1626]">{v}</span></div>
        ))}
      </div>}

      {step === 4 && <div className="card p-3 mt-4 text-[13px] text-[#5B6B82]">
        Ready to create. The full secret will be shown <b className="text-[#0E1626]">once</b> — copy it now, it cannot be viewed again. Only the masked fingerprint is stored.
      ...
      {step === 5 && secret && <div className="mt-4">
        <div className="card p-3 !border-amber-500/50 bg-amber-500/[0.07]">
          <div className="text-[#92400E] text-[13px] font-medium">Copy this key now. You will not be able to view it again.</div>
          <div className="mono text-[#0E1626] text-[14px] mt-2 break-all select-all">{secret}</div>
...
function RotateModal({ k, onClose }: { k: ApiKeyMeta; onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="text-[#0E1626] font-medium">Rotate key</div>
      {!done ? <>
        <div className="card p-3 mt-3 text-[13px]">
          <div className="flex justify-between py-1"><span className="text-[#5B6B82]">Current</span><span className="mono text-[#0E1626]">{k.fingerprint}</span></div>
          <div className="flex justify-between py-1"><span className="text-[#5B6B82]">New key</span><span className="text-[#0E1626]">created on confirm (masked)</span></div>
          <div className="flex justify-between py-1"><span className="text-[#5B6B82]">Grace period</span><span className="text-[#0E1626]">24h</span></div>
          <div className="flex justify-between py-1"><span className="text-[#5B6B82]">Old key</span><span className="text-[#B45309]">scheduled for revoke</span></div>
        </div>
        ...
        <div className="card p-3 mt-3 text-[13px] text-[#5B6B82]">New fingerprint issued. Old key stays valid for <b className="text-[#0E1626]">24h</b>, then auto-revokes. Rotation logged to Audit.</div>
...
function RevokeModal({ k, onClose }: { k: ApiKeyMeta; onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="text-[#0E1626] font-medium">Revoke key <span className="mono text-[#5B6B82]">{k.name}</span></div>
      <div className="card p-3 mt-3 text-[13px]">
        {[['Owner', k.owner], ['Provider', k.provider], ['Affected apps', String(k.apps)], ['Last used', k.lastUsed]].map(([a, b]) => (
          <div key={a} className="flex justify-between py-1 border-b border-[#E8EDF4] last:border-0"><span className="text-[#5B6B82]">{a}</span><span className="text-[#0E1626]">{b}</span></div>
        ))}
      </div>
      <div className="card p-3 mt-2 !border-red-500/50 bg-red-500/[0.07] text-[13px] text-[#B91C1C]">Requests using this key will stop immediately.</div>
...
function RequestKeyModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} wide>
      <div className="text-[#0E1626] font-medium">Request key</div>
    ...
    <div>
      ...
      <div className="flex gap-1.5 mb-3">
        {([['keys', `Keys (${keys.length})`], ['requests', `Requests${pending ? ` (${pending} pending)` : ''}`], ['revoked', `Revoked (${revokedKeys.length})`]] as [Tab, string][]).map(([t, l]) => (
          <button key={t} className={`btn ${tab === t ? '!border-blue-500 !text-[#0E1626]' : ''}`} onClick={() => setTab(t)}>{l}</button>
        ))}
      ...
      {tab === 'keys' && (
        <div className="card overflow-hidden">
          <table className="w-full text-[12.5px]">
            <thead><tr className="text-left text-[#5B6B82] text-[11px] uppercase border-b border-[#E3E9F2]">
            ...
            <tbody>{rows.map(k => (
              <tr key={k.id} className="table-row border-b border-[#E8EDF4]">
                <td className="p-2.5"><span className="mono text-[#0E1626]">{k.name}</span><div className="mono text-[11px] text-[#8FA0B5]">{k.fingerprint}</div></td>
                <td>{k.provider}</td><td>{k.owner}</td><td><span className="badge">{k.env}</span></td><td className="mono">{k.scope}</td>
                <td className="text-[#5B6B82]">{k.created}</td><td className="text-[#5B6B82]">{k.lastUsed}</td>
          ...
          {rows.length === 0 && <div className="p-6 text-center text-[13px] text-[#5B6B82]">No keys match. <button className="text-[#2470D8]" onClick={() => setWizard(true)}>Create one →</button></div>}
        </div>
      )}
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-[12.5px]">
            ...
            <tbody>{keyRequests.map(r => (
              <>
                <tr key={r.id} className="table-row border-b border-[#E8EDF4]">
                  <td className="p-2.5"><span className="mono text-[#0E1626]">{r.id}</span><div className="text-[11px] text-[#8FA0B5]">{r.requester} · {r.date}</div></td>
                  <td>{r.team}</td><td>{r.provider}<div className="text-[11px] text-[#8FA0B5]">{r.model}</div></td>
                  <td className="max-w-[220px]">{r.purpose}</td><td className="mono">{r.scope} · {r.env}</td><td>{r.budget}</td>
                  <td><span className={`text-[12px] ${r.status === 'Pending' ? 'text-[#B45309]' : r.status === 'Approved' ? 'text-[#047857]' : 'text-[#DC2626]'}`}>{r.status}</span></td>
                  <td className="p-2.5"><div className="flex justify-end gap-1">
                ...
                {viewReq === r.id && (
                  <tr key={r.id + '-d'} className="border-b border-[#E8EDF4] bg-[#EDF2F8]">
                    <td colSpan={8} className="p-3 text-[12.5px]">
                      <div className="grid md:grid-cols-4 gap-2">
                        {[['Expected usage', r.usage], ['Budget / quota', r.budget], ['Environment', r.env], ['Requested', r.date]].map(([a, b]) => (
                          <div key={a} className="card p-2.5"><div className="kpi-label">{a}</div><div className="text-[#0E1626] mt-0.5">{b}</div></div>
                        ))}
                      </div>
                      <div className="text-[#5B6B82] mt-2">Approval creates a key + audit event. Flow: team request → manager approval → key → audit.</div>
                    </td>
                  </tr>
                )}
              </>
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-[12.5px]">
            ...
            <tbody>{revokedKeys.map(k => (
              <tr key={k.id} className="table-row border-b border-[#E8EDF4]">
                <td className="p-2.5 mono text-[#0E1626]">{k.name}</td><td>{k.owner}</td><td>{k.provider}</td>
                <td className="text-[#5B6B82]">{k.revokedBy}</td><td className="text-[#5B6B82]">{k.revokedAt}</td>
                <td>{k.reason}</td><td className="text-[#5B6B82]">{k.lastUsed}</td>
              </tr>
            ))}</tbody>
          </table>
          <div className="px-3 py-2 text-[11.5px] text-[#8FA0B5]">No restore by default — create a new key instead. Every revocation is in <Link className="text-[#2470D8]" to="/audit">Audit Logs →</Link></div>
        </div>
      )}
      ...
      {view && (
        <Modal onClose={() => setView(null)}>
          <div className="text-[#0E1626] font-medium mono">{view.name}</div>
          <div className="card p-3 mt-3 text-[13px]">
            {[['Fingerprint', view.fingerprint], ['Provider', view.provider], ['Owner', view.owner], ['Environment', view.env], ['Scope', view.scope], ['Models', view.models.join(', ')], ['Quota', view.quota], ['Budget', view.budget], ['Expires', view.expires], ['Status', view.status], ['Affected apps', String(view.apps)]].map(([a, b]) => (
              <div key={a} className="flex justify-between py-1 border-b border-[#E8EDF4] last:border-0"><span className="text-[#5B6B82]">{a}</span><span className="text-[#0E1626] mono">{b}</span></div>
            ))}
          </div>
          <div className="text-[12px] text-[#8FA0B5] mt-2">Full secret never displayed after creation — rotate if lost.</div>
