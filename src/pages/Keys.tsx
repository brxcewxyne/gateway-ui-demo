...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block text-[12px] text-[#8b94a3]">{label}<div className="mt-1">{children}</div></label>;
}
...
function CreateWizard({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} wide>
      <div className="text-white font-medium text-[15px]">Create key {step <= 4 && <span className="text-[#8b94a3] font-normal">· step {step}/4</span>}</div>
      {step <= 4 && <div className="flex gap-1 mt-2">{[1, 2, 3, 4].map(i => <div key={i} className={`h-1 flex-1 rounded ${i <= step ? 'bg-blue-500' : 'bg-[#243044]'}`} />)}</div>}
      ...
      {step === 3 && <div className="card p-3 mt-4 text-[13px]">
        {[['Name', form.name || '(auto)'], ['Provider', form.provider], ['Team', form.team], ['Environment', form.env], ['Models', form.models], ['Scope', form.scope], ['Quota', form.quota], ['Budget', form.budget], ['Expires', form.expires]].map(([k, v]) => (
          <div key={k} className="flex justify-between py-1 border-b border-[#1a2230] last:border-0"><span className="text-[#8b94a3]">{k}</span><span className="text-white">{v}</span></div>
        ))}
      </div>}

      {step === 4 && <div className="card p-3 mt-4 text-[13px] text-[#8b94a3]">
    ...
    <Modal onClose={onClose}>
      ...
      {!done ? <>
        <div className="card p-3 mt-3 text-[13px]">
          <div className="flex justify-between py-1"><span className="text-[#8b94a3]">Current</span><span className="mono text-white">{k.fingerprint}</span></div>
          <div className="flex justify-between py-1"><span className="text-[#8b94a3]">New key</span><span className="text-white">created on confirm (masked)</span></div>
          <div className="flex justify-between py-1"><span className="text-[#8b94a3]">Grace period</span><span className="text-white">24h</span></div>
          <div className="flex justify-between py-1"><span className="text-[#8b94a3]">Old key</span><span className="text-amber-300">scheduled for revoke</span></div>
        </div>
        ...
        <div className="card p-3 mt-3 text-[13px] text-[#8b94a3]">New fingerprint issued. Old key stays valid for <b className="text-white">24h</b>, then auto-revokes. Rotation logged to Audit.</div>
...
function RevokeModal({ k, onClose }: { k: ApiKeyMeta; onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="text-white font-medium">Revoke key <span className="mono text-[#8b94a3]">{k.name}</span></div>
      <div className="card p-3 mt-3 text-[13px]">
        {[['Owner', k.owner], ['Provider', k.provider], ['Affected apps', String(k.apps)], ['Last used', k.lastUsed]].map(([a, b]) => (
          <div key={a} className="flex justify-between py-1 border-b border-[#1a2230] last:border-0"><span className="text-[#8b94a3]">{a}</span><span className="text-white">{b}</span></div>
        ))}
      </div>
...
export default function Keys() {
  ...
  return (
    <div>
      <PageHead title="Keys" sub="Application and provider access. Secrets shown once at creation — only masked fingerprints are stored."
        right={<>{tab === 'requests'
      ...
      {tab === 'keys' && (
        <div className="card overflow-hidden">
          <table className="w-full text-[12.5px]">
            <thead><tr className="text-left text-[#8b94a3] text-[11px] uppercase border-b border-[#1f2733]">
            ...
            <tbody>{rows.map(k => (
              <tr key={k.id} className="table-row border-b border-[#161d28]">
                <td className="p-2.5"><span className="mono text-white">{k.name}</span><div className="mono text-[11px] text-[#5b6575]">{k.fingerprint}</div></td>
                <td>{k.provider}</td><td>{k.owner}</td><td><span className="badge">{k.env}</span></td><td className="mono">{k.scope}</td>
                <td className="text-[#8b94a3]">{k.created}</td><td className="text-[#8b94a3]">{k.lastUsed}</td>
          ...
          {rows.length === 0 && <div className="p-6 text-center text-[13px] text-[#8b94a3]">No keys match. <button className="text-blue-400" onClick={() => setWizard(true)}>Create one →</button></div>}
        </div>
      )}
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-[12.5px]">
            ...
            <tbody>{keyRequests.map(r => (
              <>
                <tr key={r.id} className="table-row border-b border-[#161d28]">
                  <td className="p-2.5"><span className="mono text-white">{r.id}</span><div className="text-[11px] text-[#5b6575]">{r.requester} · {r.date}</div></td>
                  <td>{r.team}</td><td>{r.provider}<div className="text-[11px] text-[#5b6575]">{r.model}</div></td>
                ...
                {viewReq === r.id && (
                  <tr key={r.id + '-d'} className="border-b border-[#161d28] bg-[#0e141d]">
                    <td colSpan={8} className="p-3 text-[12.5px]">
                      ...
                      <div className="text-[#8b94a3] mt-2">Approval creates a key + audit event. Flow: team request → manager approval → key → audit.</div>
                    </td>
                  </tr>
                )}
              </>
          ...
          <table className="w-full text-[12.5px]">
            ...
            <tbody>{revokedKeys.map(k => (
              <tr key={k.id} className="table-row border-b border-[#161d28]">
                ...
                <td className="text-[#8b94a3]">{k.revokedBy}</td><td className="text-[#8b94a3]">{k.revokedAt}</td>
                <td>{k.reason}</td><td className="text-[#8b94a3]">{k.lastUsed}</td>
              </tr>
            ))}</tbody>
          </table>
          <div className="px-3 py-2 text-[11.5px] text-[#5b6575]">No restore by default — create a new key instead. Every revocation is in <Link className="text-blue-400" to="/audit">Audit Logs →</Link></div>
        </div>
      )}
        ...
        <Modal onClose={() => setView(null)}>
          ...
          <div className="card p-3 mt-3 text-[13px]">
            {[['Fingerprint', view.fingerprint], ['Provider', view.provider], ['Owner', view.owner], ['Environment', view.env], ['Scope', view.scope], ['Models', view.models.join(', ')], ['Quota', view.quota], ['Budget', view.budget], ['Expires', view.expires], ['Status', view.status], ['Affected apps', String(view.apps)]].map(([a, b]) => (
              <div key={a} className="flex justify-between py-1 border-b border-[#1a2230] last:border-0"><span className="text-[#8b94a3]">{a}</span><span className="text-white mono">{b}</span></div>
            ))}
          </div>
          <div className="text-[12px] text-[#5b6575] mt-2">Full secret never displayed after creation — rotate if lost.</div>
