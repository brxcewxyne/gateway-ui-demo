...
export default function Models() {
  ...
  return (
    <div>
      <PageHead title="Models — supporting the routing layer" sub="Registry, health and kill-switches. Priority here drives Routing fallback order."
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-[13px]">
          <thead><tr className="text-left text-[#8b94a3] text-[11.5px] uppercase tracking-wider border-b border-[#1f2733]"><th className="p-3">Model</th><th>Cost/1M</th><th>p95</th><th>Req</th><th>Health</th><th>Priority</th><th>Status</th><th className="text-right pr-3">Actions</th></tr></thead>
          <tbody>
            {rows.map(m => (
              <>
                <tr key={m.id} className="table-row border-b border-[#161d28]">
                  <td className="p-3"><button className="text-white font-medium hover:text-blue-300" onClick={() => setOpen(open === m.id ? null : m.id)}>{m.name}</button><div className="text-[11.5px] text-[#8b94a3]">{m.provider} · {m.capabilities.join(' · ')}</div></td>
                ...
                {open === m.id && (
                  <tr key={m.id + '-d'} className="border-b border-[#161d28] bg-[#0e141d]">
                    <td colSpan={8} className="p-3">
                      <div className="grid md:grid-cols-4 gap-2 text-[12.5px]">
                        ...
                        <div className="card p-2.5"><div className="kpi-label">Quality vs cost</div><div className="text-white">{'★'.repeat(m.quality)}{'☆'.repeat(5 - m.quality)} · {fmt$(m.costPer1M)}/1M</div><div className="text-[#8b94a3] text-[12px]">Routing prefers this when difficulty justifies it.</div></div>
                      </div>
                    </td>
                  </tr>
                )}
